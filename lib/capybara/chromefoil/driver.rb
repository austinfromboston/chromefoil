...
module Capybara::Chromefoil
  class Driver < Capybara::Driver::Base
    ...
    def server
      @server ||= DevToolsClient.new(options[:port], options.fetch(:timeout) { DEFAULT_TIMEOUT }, options[:host])
    end

    def client
      @client ||= ChromeClient.start(server)
      #@client ||= ChromeClient.start(server,
        #:path              => options[:phantomjs],
        #:window_size       => options[:window_size],
        #:chrome_options => phantomjs_options,
        #:chrome_logger  => phantomjs_logger
      #)
    end

    def chrome_options
      list = options[:chrome_options] || []
    end
    ...
    def chrome_logger
      options.fetch(:chrome_logger, nil)
    end
    ...
    def find(method, selector)
      browser.find(method, selector).map { |page_id, id| Capybara::Chromefoil::Node.new(self, page_id, id) }
    end
    ...
    def evaluate_script(script, *args)
      result = browser.evaluate(script, *args.map { |arg| arg.is_a?(Capybara::Chromefoil::Node) ?  arg.native : arg})
      unwrap_script_result(result)
    end

    def execute_script(script, *args)
      browser.execute(script, *args.map { |arg| arg.is_a?(Capybara::Chromefoil::Node) ?  arg.native : arg})
      nil
    end
    ...
    def pause
      ...
      STDERR.puts "Chromefoil execution paused. Press enter (or run 'kill -CONT #{Process.pid}') to continue."
    ...
    def invalid_element_errors
      []
      #[Capybara::Chromefoil::ObsoleteNode, Capybara::Chromefoil::MouseEventFailed]
    end
    ...
    def unwrap_script_result(arg)
      ...
      when Hash
        return Capybara::Chromefoil::Node.new(self, arg['ELEMENT']['page_id'], arg['ELEMENT']['id']) if arg['ELEMENT']
