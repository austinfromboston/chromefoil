module Capybara
  module Chromefoil
    ...
    class BrowserError < ClientError
      ...
      def message
        "There was an error inside Google Chrome. " \
    ...
    class JavascriptError < ClientError
      ...
      def message
        "One or more errors were raised in the Javascript code on the page. " \
          ...
          "setting js_errors: false in your configuration (see " \
    ...
    class MouseEventFailed < NodeError
      ...
      def message
        "Firing a #{name} at co-ordinates [#{position.join(', ')}] failed. Chromefoil detected " \
    ...
    class TimeoutError < Error
      ...
      def message
        "Timed out waiting for response to #{@message}. It's possible that this happened " \
          ...
          "If so, setting the :timeout option to a higher value will help " \
          "(see the docs for details). If increasing the timeout does not help, this is " \
          "probably a bug in Chromefoil - please report it to the issue tracker."
      end
    end
